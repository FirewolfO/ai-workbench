...
- 图片及文件附件随单次消息发送；消息中保留图片缩略图，模型调用读取后立即从服务器删除原文件
...
每次普通对话还会向模型提供后台生活工具：天气与 7 天预报、IANA 时区时间、计算器、常用单位换算、参考汇率和公共节假日。天气使用 Open-Meteo，汇率使用 Frankfurter，节假日使用 Nager.Date；这些固定上游不需要用户 API Key，也不会接受模型提供的任意 URL。支持函数工具的模型会自行判断是否调用并在取得结果后组织回答；旧网关若明确拒绝函数工具字段，系统会自动回退到普通对话，避免影响原有模型可用性。
...
连接的“启用”与“可使用”状态相互独立。管理员保存连接后必须执行“测试连接”；只有模型列表和最小对话请求都成功的已启用连接，才会标记为“可使用”并出现在聊天模型列表。测试失败或修改 Base URL、默认模型、API Key 后，连接会退出可用列表，直至重新测试成功。
